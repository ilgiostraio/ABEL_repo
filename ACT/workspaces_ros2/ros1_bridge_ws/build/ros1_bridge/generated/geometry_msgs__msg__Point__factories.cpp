...
get_factory_geometry_msgs__msg__Point(const std::string & ros1_type_name, const std::string & ros2_type_name)
{
    ...
    ros2_type_name == "geometry_msgs/msg/Point")
  {
    return std::make_shared<
      ...
      >
    >("geometry_msgs/Point", ros2_type_name);
  }
  return std::shared_ptr<FactoryInterface>();
}

std::unique_ptr<ServiceFactoryInterface>
get_service_factory_geometry_msgs__msg__Point(const std::string & ros_id, const std::string & package_name, const std::string & service_name)
{
  (void)ros_id;
  (void)package_name;
  (void)service_name;
