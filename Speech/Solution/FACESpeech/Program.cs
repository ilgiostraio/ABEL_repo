...
namespace FACESpeech
{
    class Program
    {
        ...
        private static void InitYarp()
        {
            ...
            yarpPortSpeech.openReceiver("/AttentionModule/Speech:o", "/RobotSpeech/Speech:i");

            yarpPortFeedBackSpeech = new YarpPort();
            yarpPortFeedBackSpeech.openSender("/RobotSpeech/FeedBackSpeech:o");

            yarpPortLipAnimator = new YarpPort();
            yarpPortLipAnimator.openSender("/RobotSpeech/LipAnimation:o");
        ...
        static void ReceiveDataSpeech(object sender)
        {
            while (true)
            {
                ...
                if (receiveLookAtData != null && receiveLookAtData != "")
                {
                    try
                    {
                        fileAudio = receiveLookAtData.Replace("\"","");
                        LipAnimation();
                    ...
                    {
                        Console.WriteLine("Error ReceiveDataSpeech: " + exc.Message);
                    }
        ...
        static void LipAnimation()
        {
            ...
            using (var reader = new AudioFileReader("audio/"+fileAudio+".wav"))
            {
                ...
                wave.Add((float)Math.Truncate(timeSample * 1000));
                ...
                do
                {
                    ...
                    for (int n = 0; n < read; n++)
                    {
                        ...
                        float abs = (float) Math.Truncate(10000 * Math.Abs(buffer[n])) / 10000;
