...
namespace AttentionModule
{
    ...
    public class AttentionModuleDef : Module, IDisposable
    {
        ...
        public CLIPSNet.DataTypes.Integer FunLookAt(CLIPSNet.DataTypes.Integer id, CLIPSNet.DataTypes.Double xCoord, CLIPSNet.DataTypes.Double yCoord)
        {
            ...
            if(id.Value!=1)
            {
                foreach (Subject subject in sceneData.Subjects) //calibration of the LookAt point
                {
                    ...
                    {
                        //Xmax = subject.head.Z * (float)Math.Tan((35.00 / 180.00) * Math.PI); //alzando l'angolo della tangente aumento l'eccentricità
                        //X = ((subject.head.X / Xmax) / 2) + (float)0.45; //con il + calibro uno spostamento a destra del punto

                        //Ymax = subject.head.Z * (float)Math.Tan((30.00 / 180.00) * Math.PI);
                        //Y = ((subject.head.Y / Ymax) / 2) + (float)0.5; //

                        Xmax = subject.head.Z * (float)Math.Tan(42.00 / 180.00 * Math.PI); //alzando l'angolo della tangente aumento l'eccentricità
                        X = (subject.head.X / Xmax / 2) + (float)0.5; //con il + calibro uno spostamento a destra del punto, 0.5 è il numero per la seduta, 0.76 per lo stand

                        Ymax = subject.head.Z * (float)Math.Tan(26.50 / 180.00 * Math.PI);
                        Y = (subject.head.Y / Ymax / 2) + (float)0.5;
            ...
            {
                if (!(WinnerSub.spinX == WinnerSubOld.spinX && WinnerSub.spinY == WinnerSubOld.spinY))
                {
        ...
        {
            if (!close)
                moodPort.sendData(Convert.ToString(posture));
